...
export const BIOMAS_REGIONALES_AMERICA: Record<string, BiomaFicha> = {
  ...
  alaska_costa_taiga: {
    ...
    suelos: 'Spodosoles muy lavados bajo la selva costera, con un manto orgánico grueso y a veces una capa de hierro cementada que estanca el agua: sin perturbación el bosque se va convirtiendo en turbera solo. Tierra adentro son Gelisoles, y ahí el musgo no es adorno sino el aislante que mantiene el permafrost abajo; si se raspa o se compacta con maquinaria, el hielo se derrite y el terreno se hunde. Lo que manda la temporada no es la temperatura del aire sino la del suelo, que tarda semanas más en despertar.',
    saberes: [],
    especies: [
      'Sitka spruce (Picea sitchensis)',
      'Tsuga occidental (Tsuga heterophylla)',
      'Cedro amarillo (Callitropsis nootkatensis)',
      'Black spruce (Picea mariana)',
      'Abedul de Alaska (Betula neoalaskana)',
      'Álamo balsámico (Populus balsamifera)',
  ...
  alaska_tundra_hielo_beringia: {
    ...
    suelos: 'Gelisoles con una capa activa de treinta a sesenta centímetros sobre permafrost, revuelta por crioturbación: el hielo mezcla los horizontes cada año, y por eso la materia orgánica aparece enterrada en profundidad en vez de arriba. Hay stock de nutrientes, lo que falta es temperatura para liberarlos. La huella de un vehículo en verano rompe el aislante y se vuelve un canal que sigue ensanchándose por décadas. En las Aleutianas no hay permafrost sino ceniza volcánica, y ahí sí hay pradera productiva.',
    saberes: [],
    especies: [
      'Cottongrass (Eriophorum spp.)',
      'Sauce ártico (Salix arctica)',
      'Dryas de ocho pétalos (Dryas octopetala)',
  ...
  altiplano_mexicano_matorral: {
    ...
    suelos: 'Calcisoles con caliche y, sobre todo, tepetate: una capa endurecida de origen volcánico que hace de piso y que en media meseta quedó a la vista cuando se fue el suelo de arriba. Sobre tepetate desnudo no crece nada hasta que se rompe y se le mete materia orgánica, y eso es trabajo de años, no de una campaña. El pH alto deja el hierro y el zinc trabados aunque estén, y en las cuencas cerradas sin salida la sal se acumula. La lluvia es poca y violenta: lo que define el año es dónde se la frena, no cuánto guarda el perfil.',
    saberes: [],
    especies: [
      'Nopal (Opuntia spp.)',
      'Maguey (Agave spp.)',
  ...
  apalaches_bosques_y_rivercane: {
    ...
    suelos: 'Ultisoles ácidos donde lo que corta la raíz en profundidad es el aluminio, más que la falta de nutrientes: encalar es la primera medida y la que más cambia el resultado. En el Piedmont el algodón del siglo XIX se llevó quince o veinte centímetros de suelo, y lo que hoy se ara es en muchos lotes el horizonte B, esa arcilla roja que compacta y escurre. La riqueza está en las cañadas, donde el coluvio bajado de la ladera es profundo y fresco: ahí está el bosque más diverso y también la mejor tierra.',
    saberes: [],
    especies: [
      'Robles (Quercus spp.)',
      ...
      'Poro silvestre (Allium tricoccum)',
  ...
  baja_california_desiertos_y_sierras: {
    ...
    suelos: 'Aridisoles y arenas sin desarrollo sobre abanicos aluviales, muchos de ellos salinos cerca de la costa, y una costra biológica viva que es lo único que sostiene la superficie entre planta y planta: se rompe de una pasada y tarda décadas en rehacerse. El suelo profundo está sólo en los oasis del fondo de los arroyos, y ahí está toda la agricultura de la península; depende de una napa somera que el bombeo baja sin que se note hasta que la palmera se seca. En la Sierra de la Laguna el granito da un suelo grueso que sostiene un pinar aislado.',
    saberes: [],
    especies: [
      ...
      'Gobernadora (Larrea tridentata)',
      'Palma de taco (Washingtonia robusta)',
  ...
  balsas_jalisco_bosques_secos: {
    ...
    suelos: 'Leptosoles y Regosoles pedregosos en ladera, con Vertisoles y aluviales en los valles. Son suelos de poco perfil que dependen por completo de la hojarasca: la selva baja pasa medio año sin hojas y lo que la sostiene es esa manta seca, que amortigua el primer aguacero del temporal. Si se junta o se quema, la lluvia de junio se lleva el suelo de un año en una tarde. En el Balsas se suman pendientes largas sin quiebre, que es la peor geometría posible.',
    saberes: [],
    especies: [
      'Copales y cuajiotes (Bursera spp.)',
  ...
  california_klamath_sierra_valle: {
    ...
    suelos: 'El aluvión del Valle Central es profundo pero muchas veces tiene un piso cementado a sesenta o cien centímetros, heredado de terrazas viejas: es lo que sostiene las charcas vernales y lo que ahoga un monte frutal si nadie lo rompió antes de plantar. En el oeste del valle, regar sin salida de drenaje sube sales y selenio. En Klamath hay serpentina, con mucho magnesio y poco calcio y metales pesados, donde lo estándar directamente no crece. En la Sierra el suelo granítico es grueso, retiene poca agua y queda hidrófobo después de un fuego severo.',
    saberes: [],
    especies: [
      'Avellano de California (Corylus cornuta var. californica)',
      'Roble de valle (Quercus lobata)',
      'Roble azul (Quercus douglasii)',
      'Pino ponderosa (Pinus ponderosa)',
      'Cedro incienso (Calocedrus decurrens)',
  ...
  californias_chaparral_costero: {
    ...
    suelos: 'Suelos someros sobre granito, lutita o arenisca, en pendiente y con poca reserva de agua. Lo que define el manejo es lo que pasa después del fuego: las ceras de los arbustos se volatilizan y condensan unos centímetros abajo, y dejan una capa que repele el agua, así que la primera lluvia fuerte del invierno escurre entera y baja como flujo de detritos. La ventana para estabilizar es ese primer invierno. Sembrar pasto encima no ayuda y compite con el rebrote. Y si el fuego vuelve cada pocos años, el chaparral no alcanza a semillar y queda pastizal anual para siempre.',
    saberes: [],
    especies: [
      ...
      'Artemisia costera (Artemisia californica)',
  ...
  centro_mexico_volcanes_bajio: {
    ...
    suelos: 'Andosoles de ceniza en los volcanes —livianos, con retención altísima y fijación fuerte de fósforo— y Vertisoles profundos en el Bajío, que es de las mejores tierras agrícolas del país. Entremedio, el tepetate: la capa endurecida que aparece apenas se pierde el suelo de encima y que hay que romper y enmendar para recuperar, con años de por medio. Y un problema que no es de suelo sino de agua y termina siéndolo: extraer más de lo que entra hunde el terreno y abre grietas que parten canales, caminos y casas.',
    saberes: [],
    especies: [
  ...
  chiapas_bosques_montanos: {
    ...
    suelos: 'Perfiles ácidos y húmedos con un mantillo orgánico grueso que es casi toda la fertilidad del sistema: Andosoles donde hay ceniza, Acrisoles y Cambisoles en el resto. Bajo dosel cerrado esa esponja de hojarasca amortigua un temporal entero; desmontada, se mineraliza en dos o tres años y lo que queda es un subsuelo arcilloso, pobre y en pendiente. La niebla que el dosel intercepta agrega agua que ningún pluviómetro registra, y es lo primero que se pierde al abrir.',
    saberes: [],
    especies: [
      ...
      'Ciprés de Guatemala (Cupressus lusitanica)',
  ...
  gran_cuenca_meseta_colorado: {
    ...
    suelos: 'Aridisoles salinos o calcáreos en cuencas cerradas: no tienen salida al mar, así que la sal que entra se queda, y el agua de riego que vuelve la concentra. Entre los arbustos hay costra criptobiótica —hongos, líquenes y cianobacterias— que fija el suelo y el nitrógeno, y que tarda de décadas a siglos en rehacerse: una pisada dura años a la vista. Cuando se rompe entra el cheatgrass, y con él un ciclo de fuego que el sagebrush no aguanta y que cambia el sistema de forma permanente. Bajo el piñón-enebro cerrado el entresuelo queda pelado y escurre.',
    saberes: [],
    especies: [
  ...
  grandes_llanuras_pradera_alta_mixta: {
    ...
    suelos: 'Mollisoles con el horizonte oscuro más profundo del planeta, y hecho por raíces, no por hojarasca: los pastos ponen dos tercios de su biomasa abajo y ahí queda el carbono. Cien años de labranza se llevaron cerca de la mitad. Hacia el oeste aparece un horizonte de carbonatos a la profundidad exacta hasta donde llega a mojar la lluvia del año, así que el perfil es el registro del clima. Lo que lo reconstruye es raíz perenne y pastoreo con descanso; ninguna enmienda reemplaza eso.',
    saberes: [],
    especies: [
      'Bisonte (Bison bison)',
  ...
  gulf_mississippi_piney_woods: {
    ...
    suelos: 'Aluviones jóvenes que se renuevan con cada creciente, arcillas pesadas de bañado atrás de los albardones, y Ultisoles arenosos en las terrazas viejas. El drenaje decide todo: treinta centímetros de desnivel son otro suelo y otro cultivo. La raíz no llega hasta donde llega el perfil sino hasta donde llega el aire, porque la napa está alta buena parte del año. Los diques cortaron el sedimento que reponía la llanura, así que atrás de ellos el terreno se compacta y baja sin que nada lo reponga.',
    saberes: [],
    especies: [
      'Ciprés calvo (Taxodium distichum)',
  ...
  hawaii_bosques_humedos_secos: {
    ...
    suelos: 'Andisoles jóvenes sobre lava y ceniza a barlovento, y Oxisoles muy meteorizados en las islas viejas. La edad del sustrato decide qué falta: el sitio joven está limitado por nitrógeno y el viejo por fósforo, y en los dos el alófano fija el fosfato soluble, así que lo que rinde es materia orgánica y roca fosfórica, no una sal que se lava con la primera lluvia. Retienen muchísima agua y pesan poco. Sobre ʻaʻā la raíz va en bolsones; sobre pāhoehoe directamente no hay dónde.',
    saberes: [],
    especies: [
  ...
  hawaii_matorrales_altos_bajos: {
    ...
    suelos: 'Arriba del bosque hay ceniza y escoria jóvenes que drenan al instante: no falta fertilidad, falta agua retenida, y de noche el suelo se congela aunque el clima sea tropical. Nada cicatriza ahí, así que una huella de pisoteo se ve por décadas. En las islas bajas el suelo es arena de coral, alcalina y sin hierro, donde el único aporte de nutrientes es el guano: las aves marinas son literalmente la fertilidad de esas islas, y donde se pierde la colonia se pierde el suelo detrás.',
    saberes: [],
    especies: [
  ...
  interior_noroeste_palouse_willamette: {
    ...
    suelos: 'El loess del Palouse es de los limos más profundos del continente y también de los más frágiles: sin piedra y sin agregados fuertes, sobre lomas de veinte y treinta por ciento, se va donde pasa la rueda. En el Willamette el problema es el opuesto, un piso arcilloso que estanca el agua de noviembre a mayo, y el calendario de trabajo lo fija el drenaje, no la helada. En la estepa del Columbia lo que cubre el suelo entre los arbustos es una costra biológica viva: tarda décadas en rehacerse y es lo que le cierra la puerta al cheatgrass.',
    saberes: [],
    especies: [
      ...
      'Balsamorriza (Balsamorhiza sagittata)',
      'Pino ponderosa (Pinus ponderosa)',
  ...
  noreste_grandes_lagos_bosques: {
    ...
    suelos: 'Spodosoles ácidos y arenosos sobre material glaciar al norte, Alfisoles bastante más fértiles al sur, y turberas en las depresiones que el hielo dejó sin desagüe. El mosaico cambia de textura en decenas de metros, así que el promedio de la ecorregión no alcanza: acá una calicata vale más que un mapa. El podzol tiene la fertilidad en los primeros centímetros de mantillo, y si se quema o se raspa queda arena. El hielo del suelo levanta piedra nueva todos los años.',
    saberes: [],
    especies: [
      'Arce azucarero (Acer saccharum)',
  ...
  oaxaca_sierras_bosques_comunales: {
    ...
    suelos: 'Andosoles de ceniza en las partes altas —livianos, con retención de agua altísima y fijación fuerte de fósforo— sobre Acrisoles y Luvisoles de ladera, y Leptosoles pedregosos en los filos. La pendiente manda sobre todo lo demás: bajo café de sombra, terraza o milpa con barreras vivas el perfil aguanta décadas, y descubierto se va en dos temporales. El corte de un camino sobre Andosol es el arranque más común de un deslizamiento. La niebla del mesófilo es agua que ningún pluviómetro registra.',
    saberes: [],
    especies: [
      'Pinos (Pinus spp.)',
      'Encinos (Quercus spp.)',
  ...
  ozarks_transicion_bosque_pradera: {
    ...
    suelos: 'Suelos residuales de caliza y dolomía: lo que queda cuando el carbonato se disuelve es pedregullo de chert y arcilla, así que buena parte del volumen es piedra y el suelo guarda mucha menos agua de la que el espesor promete. En partes hay además un piso endurecido que corta la raíz. Y todo esto es karst: lo que se tira arriba llega al manantial en horas y sin filtrar, de modo que dónde va el pozo negro o el estiércol importa acá más que en casi ningún otro lado. El cedro que avanza es fuego sacado del sistema.',
    saberes: [],
    especies: [
  ...
  pacifico_noroeste_bosques_coniferas: {
    ...
    suelos: 'Andisoles de ceniza en las Cascadas —livianos, con retención de agua altísima y fijación fuerte de fósforo— y Spodosoles ácidos en la costa. Hay muchísima materia orgánica pero fría, así que se mineraliza despacio y el nitrógeno es el que falta: lo libera la madera que se pudre, y por eso el tronco caído grande es la cuna del bosque siguiente. En el Puget Lowland el till glaciar deja un piso compacto que estanca. Pisar en húmedo con máquina compacta el perfil por décadas, y el corte de camino es de donde salen casi todos los deslizamientos.',
    saberes: [],
    especies: [
  ...
  pacifico_sur_chiapas_bosque_seco: {
    ...
    suelos: 'Leptosoles pedregosos en ladera y Vertisoles arcillosos profundos en la Depresión Central, que es sombra de lluvia: el suelo del valle es bueno pero se agrieta en seco y se vuelve intrabajable en húmedo. La milpa acá sube a pendientes de treinta y cuarenta por ciento, y ahí la quema anual sin descanso es lo que convirtió a la región en una de las más erosionadas del país. El viento de Tehuantepec agrega lo suyo: sopla meses sobre suelo descubierto y se lleva el polvo fino, que es justo la fracción que retiene agua y nutrientes.',
    saberes: [],
    especies: [
      'Parota (Enterolobium cyclocarpum)',
      'Pochote (Ceiba aesculifolia)',
  ...
  rocosas_norte_praderas_montanas: {
    ...
    suelos: 'Mollisoles profundos de pradera en el piedemonte —horizonte oscuro sobre till glaciar y loess— y suelos forestales de montaña montados sobre la capa de ceniza del Mazama, que es de donde sale casi toda la fertilidad y el agua disponible de la ladera. Esa capa tiene decímetros, no metros: cuando el camino, la saca de madera o el sobrepastoreo la levantan, abajo hay grava y el sitio no vuelve en una vida. El deshielo, y no la lluvia de verano, es lo que reparte la humedad del año.',
    saberes: [],
    especies: [
      'Pino ponderosa (Pinus ponderosa)',
      'Abeto Douglas (Pseudotsuga menziesii)',
      ...
      'Festuca de Idaho (Festuca idahoensis)',
      'Agropiro azul (Pseudoroegneria spicata)',
    ],
  ...
  rocosas_sur_sky_islands: {
    ...
    suelos: 'Mollisoles de pradera alta, Alfisoles bajo bosque y suelos volcánicos poco desarrollados, todos en pendiente y todos delgados. Lo que define el manejo acá es el fuego: después de un incendio severo el suelo queda hidrofóbico —repele el agua en vez de absorberla— y la primera tormenta del monzón se lleva la ladera entera en forma de flujo de detritos. La ventana para estabilizar una cuenca quemada es la estación seca que sigue, no la del año siguiente.',
    saberes: [],
    especies: [
      'Pino ponderosa (Pinus ponderosa)',
      'Pino piñonero (Pinus edulis)',
      'Enebro de Alligator (Juniperus deppeana)',
      'Encino de Emory (Quercus emoryi)',
  ...
  selva_maya_yucatan: {
    ...
    suelos: 'Leptosoles calizos de diez o veinte centímetros metidos en bolsones entre la roca, alternando con arcillas pesadas en los bajos inundables. La fertilidad no está en el perfil sino en el mantillo y en la biomasa parada, y de ahí que la milpa con descanso funcione y el cultivo continuo no: al tercer año no queda de dónde. La clasificación maya del suelo —k\'ankab rojo, box lu\'um negro, tsek\'el pedregoso— es más útil que cualquier mapa porque trabaja a la escala del bolsón. Y no hay ríos: lo que se derrama llega al acuífero en horas y sin filtro.',
    saberes: [],
    especies: [
      'Ramón (Brosimum alicastrum)',
      'Chicozapote (Manilkara zapota)',
  ...
  sierras_madre_pino_encino: {
    ...
    suelos: 'Dos químicas distintas bajo el mismo bosque: la Occidental está sobre ignimbrita volcánica y da suelos ácidos, la Oriental sobre caliza y da suelos alcalinos, y eso decide qué pino y qué encino prospera en cada ladera más que la altura. El mantillo de acícula se descompone despacio y es la única reserva de nutrientes; abajo hay poco perfil y mucha pendiente. Un incendio severo lo consume y deja la roca. Los caminos mal trazados juntan el escurrimiento de media ladera en un punto y abren cárcavas que ya no se cierran.',
    saberes: [],
    especies: [
      'Pino de Chihuahua (Pinus leiophylla)',
      'Pino real (Pinus engelmannii)',
      'Pino chino (Pinus teocote)',
      'Encinos (Quercus spp.)',
      'Madroño (Arbutus spp.)',
  ...
  sonora_sinaloa_bosque_seco_desierto: {
    ...
    suelos: 'Aridisoles con caliche: el horizonte petrocálcico aparece a veces a treinta centímetros y es el que decide qué se puede plantar, porque corta la raíz y el agua por igual. Arriba hay pavimento y costra que el tránsito rompe en una pasada y que tarda décadas en rehacerse. En las planicies de Sonora y Sinaloa, en cambio, hay Vertisoles y aluviales profundos donde está toda la agricultura de riego, y ahí el problema no es la sequía sino la sal que sube con cada campaña.',
    saberes: [],
    especies: [
  ...
  sudeste_sabanas_pino_largo: {
    ...
    suelos: 'Ultisoles arenosos, ácidos y muy lavados, con poquísima capacidad de retener nutrientes: lo que se aplica de golpe se va con la primera lluvia, así que acá se fertiliza poco y seguido, y sobre todo con materia orgánica. Abajo de la arena suele haber una capa que frena el agua, y por eso a un metro de desnivel cambia todo, de sabana húmeda a médano seco. El fuego frecuente quema acículas, no el suelo; el que sí lo daña es el incendio severo que llega después de décadas de mantillo acumulado sin quemar.',
    saberes: [],
    especies: [
      'Pino longleaf (Pinus palustris)',
      'Wiregrass (Aristida stricta)',
      'Saw palmetto (Serenoa repens)',
      'Roble turkey (Quercus laevis)',
      'Planta jarro (Sarracenia flava)',
  ...
  tamaulipas_texas_pastizal_mezquital: {
    ...
    suelos: 'Vertisoles arcillosos que se hinchan y se agrietan con la estación: en seco las grietas se tragan la primera lluvia entera antes de que el perfil llegue a mojarse, y en húmedo se vuelven pasta. La ventana para trabajarlos se mide en días, no en semanas. Se les suman Mollisoles calizos, las arenas del sand sheet y suelos salino-sódicos hacia las lagunas. El mezquital que avanza es consecuencia del sobrepastoreo y de haber sacado el fuego del sistema, no la causa del problema.',
    saberes: [],
    especies: [
      ...
      'Palma de Tamaulipas (Sabal mexicana)',
      'Nopal rastrero (Opuntia engelmannii)',
  ...
  tehuacan_cuicatlan_matorral: {
    ...
    suelos: 'Suelos someros sobre caliza y yeso, pedregosos, alcalinos y con muy poca materia orgánica, con aluviones profundos sólo en los fondos de valle. El agua es lo escaso y el suelo no la guarda: lo único que se puede hacer es frenarla y hacerla infiltrar donde hay perfil, que es exactamente lo que las obras prehispánicas del valle —canales, terrazas, presas de piedra— vienen haciendo hace milenios. Copiar el diseño local rinde más que cualquier obra importada, y el yeso además limita qué crece.',
    saberes: [],
    especies: [
      'Tetecho (Neobuxbaumia tetetzo)',
      'Cardón (Cephalocereus columna-trajani)',
      'Sotolín (Beaucarnea gracilis)',
      'Agaves (Agave spp.)',
  ...
  veracruz_tabasco_selvas_humedales: {
    ...
    suelos: 'En la planicie hay suelos orgánicos e hidromórficos que guardan carbono mientras estén inundados y lo pierden apenas se drenan: se oxidan, el terreno baja, y donde hay pirita en los sedimentos costeros el agua se acidifica al punto de matar la raíz y el pez. Lo que fertiliza esa llanura es el pulso de creciente, y lo que la arruina es el drenaje. En Los Tuxtlas, en cambio, el suelo volcánico es de los más fértiles del país, y en la montaña húmeda son perfiles ácidos que dependen enteramente del mantillo.',
    saberes: [],
    especies: [
      'Ceiba (Ceiba pentandra)',
      'Ramón (Brosimum alicastrum)',
