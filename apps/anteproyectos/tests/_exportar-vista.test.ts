...
it('exporta la vista previa de los anteproyectos', () => {
  ...
  const bloques = perfiles
    .map(p => {
      ...
      return `<section>
        <h2>${ap.titulo}</h2>
        <p class="meta">${ap.ancho_m.toFixed(1)} × ${ap.profundo_m.toFixed(1)} m · ${ap.area_total_m2} m² · alero ${ap.alero_m.toFixed(2)} m · h. muro ${ap.altura_muro_m.toFixed(2)} m · fachada ${ap.fachadaPrincipal} · muro ${ap.tecnicaMuro} ${(ap.espesorMuro_m * 100).toFixed(0)} cm</p>
