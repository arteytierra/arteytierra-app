/**
 * arcoSolar.ts — Trayectoria del sol proyectada sobre el mapa de selección de
 * sitio (Fase A4). Portado tal cual de `apps/terreno/lib/arco_solar.ts`
 * (módulo autocontenido, sin dependencias del resto de terreno) — 3 fechas
 * clave: solsticio de verano, equinoccios, solsticio de invierno. Proyección
 * azimutal equidistante: centro = cénit, borde = horizonte.
 *
 * Fórmulas: Cooper (1969) para declinación, hora solar verdadera.
 * Azimut: atan2 desde norte, sentido horario (0=N, 90=E, 180=S, 270=O) —
 * misma convención que `lib/bioclima/posicionSolar.ts` (A2), pero se
 * mantiene esta implementación separada porque el propósito es distinto:
 * dibujar el arco sobre el mapa (coordenadas lat/lng), no evaluar sombra en
 * la escena local (coordenadas x/y en metros).
 */
...
export interface ArcoSolarDelDia {
  fecha: FechaArco;
  label: string;
  labelCorto: string;
  color: string;
...
function posicionSolar(lat_deg: number, doy: number, hora_solar: number): { elevacion: number; azimut: number } | null {
  const phi = lat_deg * DEG;
  const decl = declinacion(doy);
  const H = (hora_solar - 12) * 15 * DEG;
...
function horasSalida(lat_deg: number, doy: number): { salida: number; puesta: number } {
  ...
  if (cosWs <= -1) return { salida: 0, puesta: 24 };
  if (cosWs >= 1) return { salida: 12, puesta: 12 };
...
export function calcularArcoSolar(lat: number, lng: number, radio_m: number): DatosArcoSolar {
  ...
  const arcos: ArcoSolarDelDia[] = (Object.keys(FECHAS_META) as FechaArco[]).map(fecha => {
