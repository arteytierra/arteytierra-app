...
// ─── NASA POWER API ───────────────────────────────────────────────────────────
//
// Nota: `apps/terreno` además calibra la precipitación con CHIRPS (~5 km) vía
// `/api/precipitacion`, porque la grilla de POWER (~50 km) subestima lluvia en
// terreno quebrado. Esa calibración no se portó todavía a esta app — si hace
// falta, portar `obtenerPrecipCHIRPS` y la ruta `/api/precipitacion` desde
// `apps/terreno/lib/clima.ts` y `apps/terreno/app/api/precipitacion/route.ts`.
// `aplicarCalibracionPrecip` ya está acá, lista para recibir esa calibración.
